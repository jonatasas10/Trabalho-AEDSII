...
TFunc *funcionario(int cod, char *nome, char *cpf, char *data, double salario){
	TFunc *funcionario = calloc(sizeof(*funcionario),1);
...
void salva_funcionario(TFunc *func, FILE *out) {
    ...
    fwrite(&func->salario, sizeof(int), 1, out);
    fflush(out);
}

TFunc *le_funcionario(FILE *in) {
    TFunc *func = calloc(sizeof(*func),1);
   ...
   // printf("result %d func->cod %d \n" , result, func->cod);
    ...
    fread(&func->salario, sizeof(int), 1, in);
...
void cria_base_dados_funcionarios_teste(FILE* arq, FILE* arqIndicePrimario,  FILE* arqIndicePrimarioNaoOrdenado, int nFunc){
    ...
    free(func1);
    i++;
    TFunc *func3 = funcionario(6,"Italo","111.111.111-11","09/10/1986",10000);
    fseek(arq, (i) * tamanho_registro(), SEEK_SET);
    ...
    free(func3);
    i++;
    TFunc *func2 = funcionario(0,"Rosangela","222.222.222-22","09/08/1970",3000);
    fseek(arq, (i) * tamanho_registro(), SEEK_SET);
    ...
    free(func4);
    i++;
    TFunc *func6 = funcionario(1,"Adriana","555.555.555-55","26/11/1988",1000);
    fseek(arq, (i) * tamanho_registro(), SEEK_SET);
    ...
    free(func19);
    i++;
    TFunc *func20 = funcionario(3,"Paula","999.999.999-99","27/05/1975",1000);
    fseek(arq, (i) * tamanho_registro(), SEEK_SET);
...
void imprime_base_dados_funcionarios(FILE *arq, int nFunc) {
    ...
    TFunc *v;
    for (int i = 0; fgetc(arq) != EOF; i++){
        fseek(arq, i*tamanho_registro(), SEEK_SET);
        v = le_funcionario(arq);

        //para a seleção natural imprimir direito quando intercala com algoritmo básico
        imprime_funcionario(v);
    }
    free(v);
